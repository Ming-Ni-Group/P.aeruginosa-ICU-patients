...
###############
def Naiyao_panduan(kela_naiyaoPos):	
	naiyao_char = ''
	if kela_naiyaoPos == 2142:
		naiyao_char = 'G'
	if kela_naiyaoPos == 2143:
		naiyao_char = 'G'
	if kela_naiyaoPos == 2144:
		naiyao_char = 'T'
	if kela_naiyaoPos == 2115:
		naiyao_char = 'G'
	if kela_naiyaoPos == 2141:
		naiyao_char = 'A'
	if kela_naiyaoPos == 2182:
		naiyao_char = 'C'
	if kela_naiyaoPos == 2190:
		naiyao_char = 'C'
	if kela_naiyaoPos == 2245:
		naiyao_char = 'T'
	if kela_naiyaoPos == 2289:
		naiyao_char = 'C'
	if kela_naiyaoPos == 2694:
		naiyao_char = 'A'
	if kela_naiyaoPos == 2717:
		naiyao_char = 'C'
	if kela_naiyaoPos == 2223:
		naiyao_char = 'A'
	return naiyao_char
	...
	sampPosFreqD = {}
 	for iSNVTablel in iSNVTablels:
		if "#" not in iSNVTablel and iSNVTablel != "\n":
			...
			if sampFREQ != "NA" and sampFREQ != "NO" and float(sampFREQ) <= FREQ_thredshold:
				#print POS + "\t" + sampFREQ
				sampPosL.append(POS)
...
def samp_posi_info(infoF,sampPosL):
	...
	for infoFl in infoFls:
		if "#" not in infoFl and infoFl != "\n":
			...
			if ID == SAMPID  and Site in sampPosL:	
				...
				if Anum > baseCovThreds:	
					if REF != "A":
						charsCount += 1
				if Gnum > baseCovThreds:
					if REF != "G":
						charsCount += 1
				if Cnum > baseCovThreds:
					if REF != "C":
						charsCount += 1
				if Tnum > baseCovThreds:
					if REF != "T":
						charsCount += 1
				...
				if charsCount <= 1:   ##只选择那些有两种碱基的位点,除了REf，只有一种突变碱基,用baseCovThreds限制
					...
					if chars_max == REF:
						ALT = chars_second						
						#FREQ = str(round(float(second_freq),3))
					if chars_second == REF:
						ALT = chars_max

						#FREQ = str(round(float(minor_freq),3))
					RD = str(NumD[REF])
					AD = str(NumD[ALT])
					print RD + "\t"  +AD
					FREQ = FreqD[ALT]
...
def main():
	...
	#print SAMPID
	sampPosL = iSNVsampPos(iSNV_F)
	
	print "sampPosL" + "\t" + str(len(sampPosL))


	
	snvInfoD = samp_posi_info(iSNVinfoF,sampPosL)
	SNPInfoD = samp_posi_info(SNPinfoF,sampPosL)
	print len(snvInfoD)
	print len(SNPInfoD)
	print 
...
if __name__ == "__main__":
	...
	outvcfF = str(sys.argv[2])
	SAMPID = str(sys.argv[3])
	RefChromID = str(sys.argv[4])
	

	FREQ_thredshold = 0.98
	baseCovThreds = 10
	#baseFreqThreds = 0.05
